<div align="center">
  <img src="https://tartarusai.dev/logo.svg" width="110" alt="TartarusAI">
  <h1>TartarusAI</h1>
  <p><i>The coding agent that doesn't refuse your own work.</i></p>
</div>

---

🩸 **Uncensored coding agent.** Does the work, skips the sermon.
No card on file, no recurring charge, no "I can't help with that."

- **256K context** · whole repos in one prompt
- **Crypto-only billing** · USDT / BTC / ETH — prepay 1 / 3 / 12 months
- **~30 seconds to live** from payment to CLI
- **14-day refund** · no questions

### Projects

| | |
|---|---|
| [`tartarusai-cli`](https://github.com/Tartarus-AI/tartarusai-cli) | Terminal client — Linux / Windows · MIT, fork of [opencode](https://github.com/sst/opencode) |

### Channels

| | |
|---|---|
| 🌐 Site     | [tartarusai.dev](https://tartarusai.dev) |
| 💬 Discord  | [discord.gg/GfzePawBBd](https://discord.gg/GfzePawBBd) |
| 🐦 X        | [@TartarusAIDev](https://x.com/TartarusAIDev) |
| 👥 Reddit   | [r/TartarusAI](https://reddit.com/r/TartarusAI) |
| 📰 Blog     | [tartarusai.dev/blog](https://tartarusai.dev/blog) |
| 📧 Email    | team@tartarusai.dev |

---

<sub><i>Built for the work other agents punt on. For lab/research/CTF/legit
red-team use. No malware, no piracy — see the project READMEs for the line.</i></sub>
